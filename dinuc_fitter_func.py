...
class Motif:
    ...
    def generate_motif_from_sites(self, sites, first_order=False):
        if len(self.consensus) != len(sites[0][0]):
            print("Error: consensus length {0} does not match sites length {1}!\nUse set_consensus() first!".format(
                len(self.consensus), len(sites[0][0])))
            return RuntimeError
    ...
    def return_DPWM(self):
        # first position
        DPWM_out = [[x for x in self.PWM[0] / 4 for y in range(4)]]
        ...
        for pos in range(1, self.n_pos):
            ...
            for idx in range(4):
                for idx_2 in range(4):
                    weights.append(
                        self.DPWM[pos - 1, 4 * idx + idx_2] * self.PWM[pos - 1, idx] / 4 * self.PWM[pos, idx_2] / 4)
            weights /= sum(weights)
            DPWM_out.append(weights)
        return np.array(DPWM_out)
        ...
        for pos in range(1, self.n_pos):
            ...
            for idx in range(4):
                ...
                for idx_2 in range(4):
                    ...
                    ### Change MS:conditional probabilities wrongly calculated? This correction seems to not work as well
                    # weights.append(self.DPWM[pos-1,  idx + 4 * idx_2] * self.PWM[pos, idx]/4)
    ...
    def save_random_matrix(self, random_file, name='', shuffle_positions=False):
        DPWM_rnd = copy.deepcopy(self.DPWM)
        for i in range(len(DPWM_rnd)):
            shuffle(DPWM_rnd[i])
        if shuffle_positions == True:
            random_order = range(len(DPWM_rnd))
            shuffle(random_order)
            temp = []
            for i in random_order:
                temp.append(list(DPWM_rnd[i]))
            DPWM_rnd = temp
        with open(random_file, 'w') as f:
            f.write('>' + name)
            f.write('\t')
            f.write(str(len(self.PWM)))
            f.write('\n')
            for pos, row in enumerate(self.PWM):
                row = map(lambda x: round(x / 4, 5), row)
                string_tmp = ''  # str(pos+1)
            ...
            f.write('>>')
            f.write('\n')
            for pos, row in enumerate(DPWM_rnd):
                row = map(lambda x: round(x, 5), row)
                string_tmp = ''  # str(pos+1)
                for elem in row:
                    string_tmp += str(elem) + '\t'
        ...
        for pos in range(self.n_pos - 1):
            ...
            for base_1 in range(4):
                for base_2 in range(4):
                    prob[base_1, base_2] = self.DPWM[pos, 4 * base_1 + base_2] * self.PWM[pos, base_1] * self.PWM[
                        pos + 1, base_2]  # probability base_1, base_2
                    cond[base_1, base_2] = self.DPWM[pos, 4 * base_1 + base_2] * self.PWM[
                        pos, base_1]  # cond. prob. base_2 | base_1
...
# A function for reading the sites of a single motif in a sequence
def predict_affinity(seq, motif, target=False, padding=False, on_target_pos=''):
    sites = annotate_sites([seq], motif, padding)
    if target == False:
        weight_out = sum([s[1] for s in sites])
    else:
        weight_out = max([s[1] for s in sites])
    if isinstance(on_target_pos, int):
        weight_out = sites[on_target_pos]
    return weight_out
...
# function to save energies
def save_energies(oligomers, motif, dest_file='', on_target_pos=''):
    n_oligomers = len(oligomers)
    affinity_measured = np.zeros(n_oligomers)
    affinity_predicted = np.zeros(n_oligomers)
    affinity_predicted_target = np.zeros(n_oligomers)
    affinity_predicted_on = np.zeros(n_oligomers)
    for idx, oligomer in enumerate(oligomers):
        affinity_measured[idx] = 1 / oligomer[1]
        affinity_predicted[idx] = predict_affinity(oligomer[0], motif)
        affinity_predicted_target[idx] = predict_affinity(oligomer[0], motif, target=True)
        affinity_predicted_on[idx] = predict_affinity(oligomer[0], motif, on_target_pos=on_target_pos)
    ...
    for idx, oligomer in enumerate(oligomers):
        ...
        pred = round(beta * affinity_predicted[idx] / scale, 3)
        pred_target = round(beta * affinity_predicted_target[idx] / scale, 3)
        pred_on = round(beta * affinity_predicted_on[idx] / scale, 3)
        list_results.append((oligomer[0], meas, pred, pred_target, meas - pred, pred_on))

    list_sorted = sorted(list_results, key=lambda el: el[-1])
    with open(dest_file, 'w') as f:
        f.write("oligo,meas,pred,pred_max_site,diff,pred_on")
        f.write('\n')
        for elem in list_sorted:
            for el in range(5):
                f.write(str(elem[el]) + ',')
...
def main(data_file, output_file, extend_filename=False, iterations=10, mode=0, print_results=True, oligomers_all=False,
         lowest=True, save_results=True):
    dest_file = False
    on_target_pos = 12

    if mode not in [0, 1, 2]:
        print(
            "Error: mode {} is not a valid input!\nChoose between 0 (standard mode, default), 1 (palindrome mode), 2 (strict palindrome mode)".format(
                mode))
        sys.exit(1)

    if not isinstance(oligomers_all, list):
    ...
    for idx in range(iterations):
        try:
            ...
            SSE_out.append(np.sqrt(SSE))
            motiv_development.append((idx, np.sqrt(SSE), motif))

        except:
            return idx
    if lowest:
        motif_low = min(motiv_development, key=lambda z: z[1])
        motif = motif_low[2]
        if extend_filename:
            output_file += str(motif_low[0] + 1) + 'iters'
    if save_results:
        motif.print_probability_file(output_file)
        np.savetxt(output_file + '.pwm', motif.PWM)
        np.savetxt(output_file + '.dpwm', motif.DPWM)
    if print_results:
        # Predict the binding energies
        if lowest:
            print ('iterations: ' + str(motif_low[0]))
        calc_SSE(oligomers, motif, print_results=False)
        print("Motif")
        motif.print_PWM()
        motif.print_DPWM()
    ...
    if isinstance(dest_file, str):
        save_energies(oligomers, motif, dest_file=dest_file, on_target_pos=on_target_pos)

    DPWM_out = motif.return_DPWM()
    if lowest:
        return (SSE_out, motif.PWM / 4, DPWM_out, motif_low[0])
    else:
        return motiv_development
